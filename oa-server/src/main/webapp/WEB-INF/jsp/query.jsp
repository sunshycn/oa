...
<html>
	<head>
		...
		<script type="text/javascript">
			$(document).ready(function() {
				...
				var curPageAttr = "currentPage";
				pageLink.each(function() {
					$(this).attr(curPageAttr, $(this).attr("href"));
					$(this).attr("href", "javascript:void(0)");
					$(this).click(function() {
						var form = $("#queryForm");
						var action = form.attr("action");
						if (action.indexOf(curPageAttr) < 0) {
							action += "&" + curPageAttr + "=" + $(this).attr(curPageAttr);
						}
	...
	<body>
	   <h2>查询报审表</h2>
		...
		<form id="queryForm" action="${contextPath}/query?pageSize=${page.pageSize}" method="post">
			<table>
		...
		<c:if test="${not empty reportFormList}">
			<table border="1">
				...
				<tr>
					<td colspan="6">
						<c:set var="paginationPreUrl" value="" scope="request"/>
