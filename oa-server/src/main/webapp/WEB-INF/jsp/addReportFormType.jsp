...
<body>
	...
	<form action="${contextPath}/reportFormType/add" method="post">
		<table>
			...
			<tr><td>需要回复该报审表的部门(Ctrl键+鼠标左键点击复选多个部门)：<select name="requiredOrgUnitIds" multiple="multiple" size="10"><c:forEach var="orgUnit" items="${orgUnitList}"><option value="${orgUnit.id}">${orgUnit.name}</option></c:forEach></select></td></tr>
